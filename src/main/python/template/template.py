...
def get_bigquery_field_nested_type(nested_field_type: Dict[str, Any]) -> str:
    if "type" not in nested_field_type:
        raise KeyError(
            'The "type" key was not found in nested type field.', nested_field_type
        )

    if nested_field_type["type"] == "array":
        if "items" not in nested_field_type:
            raise KeyError(
                'The "items" key was not found in array type field.', nested_field_type
            )
...
def get_bigquery_union_field_type(
    union_field_type: List[Union[str, Dict[str, Any]]]
) -> str:
...
def get_bigquery_field_type(
    field_type: Union[str, Dict[str, Any], List[Union[str, Dict[str, Any]]]]
) -> str:
    ...
    if isinstance(field_type, dict):
        ...
        logical_type_value: Union[
            str, Dict[str, Any], List[Union[str, Dict[str, Any]]]
        ] = field_type.get("logicalType")

        if (
            isinstance(type_value, Hashable)
            and isinstance(logical_type_value, Hashable)
    ...
    if (
        ...
        and (field_type, None) in avro_type_to_bigquery_type_map
    ):
...
def get_bigquery_field_definition(field: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if bigquery_field_definition["type"] == "RECORD":
        bigquery_field_definition["fields"] = get_bigquery_record_field_definitions(
            field
        )
...
def get_bigquery_record_field_definitions(
    avro_schema: Dict[str, Any]
) -> List[Dict[str, Any]]:
    next_avro_record_fields: List[Dict[str, Any]] = find_next_avro_record_fields(
        avro_schema
    )
    ...
    if "type" in avro_schema:
        ...
        if isinstance(avro_schema["type"], list):
            return find_next_avro_record_fields(
                [
                    field_type
                    for field_type in avro_schema["type"]
                    if field_type != "null"
                ][0]
            )
...
def deserialize_avro(record: str, schema: Dict[str, Any], encoding: Encoding):
    ...
    if encoding is Encoding.BINARY:
        raise NotImplementedError(
            "Avro binary deserialization has not been implemented yet."
        )

    raise TypeError(
        "No deserialization method is available for this type of encoding.", encoding
    )
...
class DataflowPipelineOptions(PipelineOptions):
    ...
    def _add_argparse_args(cls, parser: ArgumentParser):
        parser.add_argument(
            "--input_subscription", required=True, help="pubsub input topic"
        )
        parser.add_argument(
            "--output_table", required=True, help="bigquery output table"
        )
        parser.add_argument(
            "--use_avro_logical_types",
            required=False,
            default=True,
            help="avro logical types",
        )
...
def get_dataflow_pipeline_options() -> DataflowPipelineOptions:
    ...
    pipeline_options = PipelineOptions(parser.parse_known_args()[1], streaming=True)
...
class TransformerDoFn(DoFn):
    ...
    def process(self, record):
        try:
            ...
            record["dob"] = str(datetime.strptime(record["dob"], "%d/%m/%Y").date())
...
def get_pipeline_options(argv: Union[Sequence[str], None]) -> PipelineOptions:
    pipeline_options: PipelineOptions = PipelineOptions(
        ArgumentParser().parse_known_args(argv)[1]
    )
...
def main():
    ...
    dataflow_pipeline_options_map: Dict[
        str, Any
